...
public class CustomPlayerRegistry {
	private static final CustomPlayerRegistry INSTANCE = new CustomPlayerRegistry();

	private final HashMap<String, CustomPlayersHandler<? extends CustomPlayer>> registry = new HashMap<>();
	...
	public static void register(String identifier, CustomPlayersHandler<?> customPlayersHandlerInstance) {
		INSTANCE.registry.put(identifier, customPlayersHandlerInstance);
	}
	...
	public static CustomPlayersHandler<?> get(String identifier) {
		return INSTANCE.registry.get(identifier);
	}
	...
	public static void forEach(BiConsumer<? super String, ? super CustomPlayersHandler<?>> action) {
		INSTANCE.registry.forEach(action);
	}
	...
	public static void saveAll(SaveData save, ServerClient player) {
		for (CustomPlayersHandler<?> cps : INSTANCE.registry.values())
	...
	public static void loadAllEnter(LoadData data) {
		for (Map.Entry<String, CustomPlayersHandler<?>> entry : INSTANCE.registry.entrySet())
